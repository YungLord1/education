...
#Создание ручки(экндпоинта) для получения общей информации
@router.get('/info')
async def info():
    return {
...
#Создание ручки(экндпоинта) для получения курсов валют
...
async def currency_rate(
    ...
    #Условие для поиска конкретной валюта и чуть ниже вывод если валюта не найдена
